...
class CalicoTransportEtcd(object):
    ...
    @_handling_etcd_exceptions
    def write_subnet_info(self, subnet, prev_index=None):
        LOG.info("Write subnet info to libnetwork %s" % subnet)
        sub_id = self.gen_docker_id()
        data={"NetworkID": sub_id,
              "IPv4Data": [{
                  "Pool": subnet['cidr'],
        ...
        if prev_index is not None:
            kwargs['prevIndex'] = prev_index
        # This is standard Calico place to keep info about Networks for
        # OpenStack
        self.client.write("/calico/libnetwork/v1/%s" % sub_id, json.dumps(data), **kwargs)
        # Begin of Mirantis changes
        # Here we propagate subnet information to the Docker IPAM
        # We do not use Calico IPAM
        docker_data={ "addrSpace":"GlobalDefault",
                      "enableIPv6":False,
                      "generic":{
                          "com.docker.network.generic":{}},
                      "id":sub_id,
                      "ipamType":"default",
                      "ipamV4Config":"[{\"PreferredPool\":\""+subnet['cidr']+"\",\"SubPool\":\"\",\"Options\":null,\"Gateway\":\"\",\"AuxAddresses\":null}]",
                      "ipamV4Info":"[{\"IPAMData\":\"{\\\"AddressSpace\\\":\\\"\\\",\\\"Gateway\\\":\\\""+subnet['gateway_ip']+"/24"+
                                   "\\\",\\\"Pool\\\":\\\""+subnet['cidr']+"\\\"}\",\"PoolID\":\"GlobalDefault/"+subnet['cidr']+"\"}]",
                      "name":subnet['network_id'],
                      "networkType":"calico",
                      "persist":True,
                      "postIPv6":False}
        self.client.write("/docker/network/v1.0/network/%s" % sub_id, json.dumps(docker_data), **kwargs)
        self.client.write("/docker/network/v1.0/endpoint_count/%s" % sub_id, "{\"Count\":0}", **kwargs)
        try:
           global_default = self.client.read("/docker/network/v1.0/ipam/default/config/GlobalDefault").value
        except:
            global_default = "{\"Scope\":\"global\",\"Subnets\":{}}"
        LOG.info("Global Default is  %s" % global_default)
        g_d = json.loads(global_default)
        subnets = g_d['Subnets']
        subnets['GlobalDefault/'+subnet['cidr']] = {
            "ParentKey": {"AddressSpace": "", "Subnet": "", "ChildSubnet": ""},
            "Pool": subnet['cidr'],
            "RefCount": 1}
        LOG.info("New Global Default is  %s" %  json.dumps(g_d))

        self.client.write("/docker/network/v1.0/ipam/default/config/GlobalDefault", json.dumps(g_d), **kwargs)


        cidr_info = subnet['cidr'].split('/')

        cidr_data = {"id":"GlobalDefault/"+subnet['cidr'],
                     "sequence":"AAAAAAAAAQAAAAAAAAAA/cAAAAAAAAAAAAAAAQAAAAAAAAAAAAAABgAAAAEAAAAAAAAAAQ=="}
        self.client.write("/docker/network/v1.0/ipam/default/data/GlobalDefault/%s/%s" % (cidr_info[0], cidr_info[1]),
                          json.dumps(cidr_data), **kwargs)
        #TODO(gokrokve) This is nasty hack for Docker libnetwork. We will enforce simple security groups here
        # Neutron calico can process Security groups and properly apply on the
        # compute host but Calico Docker part has no Security groups
        # integration as Docker does not have them. Calicoctl is used to manage
        # security groups for containers completely independetly from Docker
        # In prodcution code it is necessary to add security groups handling
        # into Calico Dcoker
        sec_rules = {"id": sub_id,
                     "inbound_rules":
                         [{"action": "allow"}],
                     "outbound_rules": [{"action": "allow"}]}
        self.client.write("/calico/v1/policy/profile/%s/rules" % sub_id, json.dumps(sec_rules), **kwargs)
